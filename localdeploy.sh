...
ZIPPED_BUNDLE=bundle/liferay-dxp-digital-enterprise-tomcat-7.0-sp7-20180307180151313.zip
LICENSE=bundle/activation-key-digitalenterprisedevelopment-7.0-janverweij.xml
BASE_DIR=/tmp
...
cp -r liferay/data/20116 $BASE_DIR/liferay-dxp-digital-enterprise-7.0-sp7/data/document_library

## Change some ports in tomcat??

## Start the bundle??
read -e -p "Start the bundle? " -i "yes" answer
if [ "$answer" = "yes" ]; then
...
## Stop the bundle??
read -e -p "Stop the bundle? " -i "yes" answer
if [ "$answer" = "yes" ]; then
